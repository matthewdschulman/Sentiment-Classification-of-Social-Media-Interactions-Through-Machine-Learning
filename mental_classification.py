...
with open('data/mental_risk_keywords.csv','r') as f:
...
np_training_data = np_training[:,0]

# get the training target values
np_training_target = np_training[:,1]
...
while 1:
	...
	testing_predictions_svm = text_clf_svm.predict_proba(np_testing_data)
	...
	if testing_predictions_svm[0] > 0.5:
		print "SVM predicts that your sentence is positive: {0}\n".format(testing_predictions_svm[0])
	elif testing_predictions_svm[0] < 0.5:
		print "SVM Bayes predicts that your sentence is negative: {0}\n".format(testing_predictions_svm[0])
	else:
		print "SVM Bayes prediction unknow\n"